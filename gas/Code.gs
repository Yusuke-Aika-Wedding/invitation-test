...
function getStatus_(guestIdRaw) {
  ...
  return {
    ...
    allergyChoice: values.allergy === 'なし' ? 'なし' : (values.allergy ? 'あり' : ''),
    allergyDetails: values.allergy && values.allergy !== 'なし' ? values.allergy : '',
...
function submitResponse_(params) {
  ...
  try {
    ...
    const allergyChoice = normalizeAllergyChoice_(params.allergyChoice);
    const allergyDetails = String(params.allergyDetails || '').trim();
    const allergy = allergyChoice === 'なし' ? 'なし' : allergyDetails;
    ...
    if (!allergyChoice) throw new Error('食物アレルギーの「あり」「なし」を選択してください。');
    if (allergyChoice === 'あり' && !allergyDetails) throw new Error('アレルギーの詳細を入力してください。');
...
function fillInvitationUrls_(sheet) {
  ...
  const urls = ids.map(row => [row[0] ? getInvitationUrl_() : '']);
...
function getMainSheet_() {
  ...
  const sheet = APP_CONFIG.sheetName ? ss.getSheetByName(APP_CONFIG.sheetName) : ss.getSheets()[0];
  if (!sheet) throw new Error(`シート「${APP_CONFIG.sheetName}」が見つかりません。`);
  return sheet;
}
...
function normalizeAllergyChoice_(value) {
  const v = String(value || '').trim();
  if (v === 'あり') return 'あり';
  if (v === 'なし') return 'なし';
  return '';
